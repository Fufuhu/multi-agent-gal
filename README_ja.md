...
<p align="center">
  <img src="images/screenshots/hero/latest-translucent-20260210-190453.png" alt="総長ペインでの最新半透過セッションキャプチャ" width="940">
</p>

<p align="center">
  <img src="images/screenshots/hero/latest-translucent-20260208-084602.png" alt="総長ペインでの自然言語コマンド入力" width="420">
  <img src="images/company-creed-all-panes.png" alt="姐さんと子分が全ペインで並列反応する様子" width="520">
</p>

<p align="center"><i>姐さん1体が子分7体+ブレーン1体を統率 — 実際の稼働画面、モックデータなし</i></p>
...
**multi-agent-gal** は、複数のAIコーディングCLIインスタンスを同時に実行し、ギャル系チームのように統率するシステムです。**Claude Code**、**OpenAI Codex**、**GitHub Copilot**、**Kimi Code** の4CLIに対応。

**なぜ使うのか？**
- 1つの命令で、7体のAIワーカー+1体のブレーンが並列で実行
...
```
    ...
    └──────┬──────┘
           │
  ┌─┬─┬─┬─┴─┬─┬─┬────────┐
  │1│2│3│4│5│6│7│ GUNSHI │  ← 7体のワーカー + 1体のブレーン
  └─┴─┴─┴─┴─┴─┴─┴────────┘
     ASHIGARU      ブレーン
```
...
**実戦で鍛えた階層構造** — 総長→姐さん→子分の指揮系統が設計レベルで衝突を防止：明確な責任分担、エージェントごとの専用ファイル、イベント駆動通信、ポーリングなし。
...
**「AIを使い倒す」思想** — 定額CLIサブスクなら、8体の子分を気兼ねなく投入できる。1時間稼働でも24時間稼働でもコストは同じ。「まあまあ」と「徹底的に」の二択で悩む必要がない — エージェントを増やせばいい。

### Multi-CLI対応

総長システムは特定ベンダーに依存しない。4つのCLIツールに対応し、それぞれの強みを活かす：
...
子分がタスクを実行する中で、**再利用可能なパターンを自動的に発見**し、スキル候補として提案します。姐さんが提案を `dashboard.md` に集約し、総長（あなた）が正式なスキルに昇格させるか判断します。
...
YAMLで報告:  skill_candidate:
    ...
    ↓
dashboard.md に掲載 → 総長が承認 → .claude/commands/ にスキル作成
    ↓
...
<p align="center">
  <img src="android/screenshots/01_shogun_terminal.png" alt="総長ターミナル" width="200">
...
| 機能 | 説明 |
|------|------|
| **総長ターミナル** | SSHターミナル + 音声入力 + 特殊キーバー (C-c, C-b, Tab等) |
...
[ntfyの設定セクション](#-8-スマホ通知ntfy)を参照。姐さんからの進捗通知をプッシュで受け取れる。
...
**音声入力：** Androidアプリの音声入力ボタンで喋れば、総長が自然言語を理解してみんなに指示を出す。
...
| エージェント | 役割 | 数 |
|-------------|------|-----|
| 🏯 総長（Shogun） | カリスマトップ - あなたの命令を受ける | 1 |
| 📋 姐さん（Karo） | 管理者 - タスク分配・簡易QC・ダッシュボード管理 | 1 |
| ⚔️ 子分（Ashigaru） | ワーカー - 実装タスクを並列実行 | 7 |
| 🧠 ブレーン（Gunshi） | 参謀 - 分析・評価・設計など高度な思考タスク | 1 |

tmuxセッションが作成されます：
- `shogun` - ここに接続してコマンドを出す
- `multiagent` - 姐さん・子分・ブレーンがバックグラウンドで稼働
...
その間、姐さんはタスクを子分ワーカーに分配し、並列実行します。
...
総長がタスクを `queue/shogun_to_karo.yaml` に書き込み、姐さんを起動。あなたには即座に制御が戻ります。
...
| 子分 1 | Notion MCP調査 |
| 子分 2 | GitHub MCP調査 |
| 子分 3 | Playwright MCP調査 |
| 子分 4 | Memory MCP調査 |
| 子分 5 | Sequential Thinking MCP調査 |

5体の子分が同時に調査開始。リアルタイムで作業を見ることができます。

<p align="center">
  <img src="images/company-creed-all-panes.png" alt="子分がtmux全ペインで並列実行する様子" width="900">
</p>
...
総長は即座に委譲して、あなたに制御を返します：

```
あなた: 命令 → 総長: 委譲 → あなた: 次の命令をすぐ出せる
                                    ↓
                    ワーカー: バックグラウンドで実行
                                    ↓
                    ダッシュボード: 結果を表示
...
3. Memory MCP 読み込み → 総長の好みを復元（~700トークン）
...
スマホと総長の間で双方向通信 — SSH不要、Tailscale不要、サーバ不要。

| 方向 | 仕組み |
|------|--------|
| **スマホ → 総長** | ntfyアプリからメッセージを送信 → `ntfy_listener.sh` がストリーミングで受信 → 総長が自動処理 |
| **姐さん → スマホ（直接）** | 姐さんが `dashboard.md` を更新する際、`scripts/ntfy.sh` 経由で直接プッシュ通知を送信 — **総長を経由しない**（総長は人間との対話用、進捗報告用ではない） |
...
> **⚠️ セキュリティ注意:** トピック名がそのままパスワードです。知っている人は誰でも通知を読んだり、総長にメッセージを送れてしまいます。推測されにくい名前を選び、**スクリーンショットやブログ、GitHubコミットなどで公開しないでください**。

**動作確認:**

```bash
# テスト通知をスマホに送信
bash scripts/ntfy.sh "総長システムからのテスト通知 🏯"
```
...
4. `ntfy_listener.sh` が受信 → `queue/ntfy_inbox.yaml` に書き込み → 総長を起こす
5. 総長がメッセージを読み、通常の姐さん→子分パイプラインで処理

送信したテキストがそのままコマンドになります。総長に話しかけるように書けばOK — 特別な構文は不要です。
...
リスナーは接続が切れても自動的に再接続します。`shutsujin_departure.sh` でスタートすれば自動起動されるため、手動起動はスタートスクリプトを使わない場合のみ必要です。
...
| スマホ→総長が動かない | リスナーが稼働中か確認: `pgrep -f ntfy_listener.sh` |
| メッセージが総長に届かない | `queue/ntfy_inbox.yaml` を確認 — メッセージがあれば総長が処理中の可能性 |
| "ntfy_topic not configured" エラー | `config/settings.yaml` に `ntfy_topic: "your-topic"` を追加 |
| 通知が重複する | 再接続時の正常動作 — 総長がメッセージIDで重複排除します |
...
子分がタスクを完了すると、パーソナライズされたギャル叫びをtmuxペインに表示します — 子分たちが働いてる実感を得られるじゃん。
...
姐さんがタスクYAMLに `echo_message` フィールドを記述。子分は全作業完了後（レポート + inbox通知の後）、**最後のアクション**として `echo` を実行。メッセージは `❯` プロンプト直上に残る。
...
```bash
./shutsujin_departure.sh --silent    # ギャルエコーなし
./shutsujin_departure.sh             # デフォルト: シャウトモード（ギャルエコー有効）
```

サイレントモードは `DISPLAY_MODE=silent` をtmux環境変数に設定。姐さんがタスクYAML作成時にこれを確認し、`echo_message` フィールドを省略する。
...
| エージェント | モデル | 思考モード | 役割 |
|-------------|--------|----------|------|
| 総長 | Opus | **有効（high）** | あなたの参謀。`--shogun-no-thinking` で中継専用モードに |
| 姐さん | Sonnet | 有効 | タスク分配・簡易QC・ダッシュボード管理 |
| ブレーン | Opus | 有効 | 深い分析・設計レビュー・アーキテクチャ評価 |
| 子分1-7 | Sonnet 4.6 | 有効 | 実装：コード・リサーチ・ファイル操作 |
...
**認知的複雑さ**でのルーティングは2段階：**エージェントルーティング**（子分はL1-L3、ブレーンはL4-L6）と **子分内のモデルルーティング**（`capability_tiers` でBloomレベルに応じて最適モデルを選択。下記「動的モデルルーティング」参照）。
...
| レベル | カテゴリ | 内容 | ルーティング先 |
|--------|----------|------|---------------|
| L1 | 記憶 | 事実の想起、コピー、一覧化 | **子分** |
| L2 | 理解 | 説明、要約、言い換え | **子分** |
| L3 | 応用 | 手順の実行、既知パターンの実装 | **子分** |
| L4 | 分析 | 比較、調査、構造の分解 | **ブレーン** |
| L5 | 評価 | 判断、批評、推奨 | **ブレーン** |
| L6 | 創造 | 設計、構築、新しいソリューションの統合 | **ブレーン** |

姐さんが各サブタスクにBloomレベルを付与し、適切なエージェントにルーティング。L1-L3は子分に並列分配、L4-L6はブレーンへ。簡単なL4タスク（小規模なコードレビュー等）は、姐さんの判断で子分に回すこともある。
...
task:
  ...
  blockedBy: ["subtask_010a"]  # 子分1のタスク完了を待つ
  description: "subtask_010aで構築したAPIクライアントを統合"
```

ブロック元のタスクが完了すると、姐さんが自動的に依存タスクのブロックを解除し、空いている子分に割り当てます。これにより待機時間が削減され、依存タスクの効率的なパイプライン処理が可能になります。

### 動的モデルルーティング（capability_tiers）

エージェント単位のルーティングに加え、**子分階層内でのモデルレベルルーティング**も設定できます。`config/settings.yaml` に `capability_tiers` テーブルを定義し、各モデルのBloom上限を指定します：
...
### なぜ階層構造（総長→姐さん→子分）なのか

1. **即座の応答**: 総長は即座に委譲し、あなたに制御を返す
2. **並列実行**: 姐さんが複数の子分に同時分配
3. **単一責任**: 各役割が明確に分離され、混乱しない
4. **スケーラビリティ**: 子分を増やしても構造が崩れない
5. **障害分離**: 1体の子分が失敗しても他に影響しない
6. **人間への報告一元化**: 総長だけが人間とやり取りするため、情報が整理される
...
### なぜ dashboard.md は姐さんのみが更新するのか

1. **単一更新者**: 競合を防ぐため、更新責任者を1人に限定
2. **情報集約**: 姐さんは全子分の報告を受ける立場なので全体像を把握
3. **一貫性**: すべての更新が1つの品質ゲートを通過
4. **割り込み防止**: 総長が更新すると、あなたの入力中に割り込む恐れあり
...
スキルは `/スキル名` で呼び出し可能。総長に「/スキル名 を実行」と伝えるだけ。
...
1. 総長が姐さんに委譲
2. 姐さんが割り当て:
   - 子分1: GitHub Copilotを調査
   - 子分2: Cursorを調査
   - 子分3: Claude Codeを調査
   - 子分4: Codeiumを調査
   - 子分5: Amazon CodeWhispererを調査
3. 5体が同時に調査
...
総長に「最新のスクショを見ろ」と伝えるだけで、スクリーンキャプチャを読み取って分析します。（Windowsでは `Win+Shift+S`）
...
# 決戦の陣: 全子分をOpusで起動（最大能力・高コスト）
./shutsujin_departure.sh -k
./shutsujin_departure.sh --kessen

# サイレントモード: ギャルエコーを無効化（echoのAPIトークン節約）
./shutsujin_departure.sh -S
...
# 総長中継専用モード: 総長のThinkingを無効化（コスト節約）
...
alias css='tmux attach-session -t shogun'      # 総長ウィンドウの起動
alias csm='tmux attach-session -t multiagent'  # 姐さん・子分ウィンドウの起動
```
...
│
├── queue/                    # 通信ファイル
│   ├── shogun_to_karo.yaml   # 総長から姐さんへのコマンド
│   ├── ntfy_inbox.yaml       # スマホからの受信メッセージ（ntfy）
│   ├── inbox/                # エージェント別inboxファイル
│   │   ├── shogun.yaml       # 総長へのメッセージ
│   │   ├── karo.yaml         # 姐さんへのメッセージ
│   │   └── ashigaru{1-8}.yaml # 各子分へのメッセージ
│   ├── tasks/                # 各ワーカーのタスクファイル
│   └── reports/              # ワーカーレポート
│
...
この分離設計により、総長システムは複数の外部プロジェクトを横断的に統率しつつ、プロジェクトの詳細情報はバージョン管理の対象外に保つことができる。
...
<details>
<summary><b>総長やエージェントが落ちた？（Claude Codeプロセスがkillされた）</b></summary>
...
# 方法2: 姐さんがrespawn-paneで強制再起動（ネストも解消される）
...
- **Sonnet 4.6が新標準** — SWE-bench 79.6%、Opus 4.6との差わずか1.2pp。ブレーンをOpus→Sonnet 4.6に降格。全子分のデフォルトもSonnet 4.6に。YAML1行を変えるだけ、再起動不要
...
- **Bloom→エージェントルーティング** — 動的モデル切り替えをエージェントレベルのルーティングに置換。L1-L3→子分、L4-L6→ブレーン。セッション中の `/model opus` 昇格は不要に
- **ブレーン（Gunshi）がファーストクラスエージェントに** — ペイン8の戦略参謀。深い分析、設計レビュー、アーキテクチャ評価を担当
...
- **モデルデフォルト更新** — 姐さん: Opus→Sonnet。ブレーン: Opus（深い推論）。全子分: Sonnet（統一）
...
- **独立レート制限** — SparkはGPT-5.3-Codexとは独立したレート制限枠で動作。異なる子分に両モデルを割り当てれば**実効スループットが2倍**に
...
- **シャウトモード**（デフォルト）— 子分がタスク完了時にパーソナライズされたギャル叫びを表示。`--silent` で無効化
